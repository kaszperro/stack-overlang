...
case class FramePanel(parent: Component)
  ...
  def innerWidth = width - 3

  def innerHeight = if (focusIndicator) height - 3 else height - 1
  ...
  private def updateDimensions(newWidth: Int, newHeight: Int): Unit = {
  ...
  def drawDebug(theme: ColorScheme): Unit = {
  ...
  def drawWidgets(theme: ColorScheme): Unit = {
    ...
    val marginX = 2
    val marginY = if (focusIndicator) 2 else 1
    //var y = marginY
    var heightChanged = false
    for ((widget, i) <- widgets.zipWithIndex) {
      if (widget.needsRedraw) {
        screen.translateOffset(x = marginX + widget.offsetX, y = marginY + widget.offsetY)
        // clear widget's space first
        screen.clip(widget.innerWidth, widget.innerHeight)
  ...
  private def horizontalDepth: Int = right match {
    case None => 1
    case Some(panel) => 1 + panel.horizontalDepth
  }

  private def verticalDepth: Int = bottom match {
    case None => 1
    case Some(panel) => 1 + panel.verticalDepth
  }

  private def maxVerticalDepth: Int = right match {
    case None => verticalDepth
    case Some(panel) => math.max(verticalDepth, panel.maxVerticalDepth)
  }

  private def verticalDepths: Seq[Int] = right match {
